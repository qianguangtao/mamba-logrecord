...
import com.app.core.mvc.converter.ThrowableConverter;
...
public class Result<T> {
    ...
    private T data;
    /**
     * 异常对象，用于Log aop获取Log StackTrace完整异常信息。
     * 也返回前端throwable.toString()简化信息，方便bug定位
     */
    @JsonSerialize(converter = ThrowableConverter.class)
    ...
    public static <T> Result<List<T>> success(IPage page, Class<T> clazz) {
        return Result.<List<T>>builder()
                .code(Code.A00000.code())
                .message(Code.A00000.message())
                .data(BeanUtil.copyToList(page.getRecords(), clazz))
    ...
    public static <T> Result<List<T>> success(IPage<T> t) {
        return Result.<List<T>>builder()
                .code(Code.A00000.code())
                .message(Code.A00000.message())
                .data(t.getRecords())
                .pageSize(t.getSize())
                .pageTotal(t.getTotal())
                .pageCurrent(t.getCurrent())
                .pageCount(t.getPages())
                .build();
    }
